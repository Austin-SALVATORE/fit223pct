...
| Wall | A single flat, FILLED vertical plane, matte warm charcoal #4A4138 (the app's `--color-border-strong` register, shifted warm) — clearly visible against the app's near-black background (`--color-bg #191512`), while staying clearly darker than skin, hair and white sneakers so the figure stays the brightest thing in the frame. No gradient, no rim light, no texture, no floor line, no baseboard, no corner. It must be flush to the frame's own edge with ZERO background visible beyond it on that side, in every frame — never a free-standing column with open background around it; the gap kept for slicing belongs only *between* the two frames, and should be noticeably narrower than the spacing used for other equipment. First used in the recovery-stretch routine (28 Jul) — the first fixed environmental feature this style has carried; every prior asset is a figure floating on transparency. The wall is part of the artwork, not a background fill, and the transparency contract still holds around it. Settled after two corrections, for opposite reasons: attempt 1 (light warm-gray #C9C4BC) was too light, close to `--color-ink`, and would have been one of the brightest things on screen; attempt 2 (#221D19, the equipment charcoal's darkest end) overcorrected — nearly invisible against `--color-bg`, and additionally rendered as an unanchored floating column rather than a wall because it wasn't pushed flush to the frame edge. #4A4138 sits clearly between background and figure; the flush-edge instruction is what actually reads as architecture rather than an object. |
