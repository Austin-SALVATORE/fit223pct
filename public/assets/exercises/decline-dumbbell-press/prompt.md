# Decline Dumbbell Press — image generation prompt

| Field | Value |
|---|---|
| Exercise id | `decline-dumbbell-press` |
| Category | Chest Variants |
| Camera | `bench-side` |
| Frames | 6 |
| Equipment | Dumbbell (one per hand), Decline bench |
| Status | `planned` |
...
the dumbbell path and the elbow angle are all readable.
Equipment: two matte black hexagonal-head dumbbells with brushed chrome
knurled handles, one in each hand, plus one matte black padded bench declined
roughly 15 degrees head-down on a black steel frame, with a padded black ankle
roller at the raised foot end. The head end points toward the left of each
frame and sits lower than the feet; the shins are hooked behind the ankle
roller at the raised end. The dumbbells are held with the handles in line with
each other and the palms facing forward toward the feet.
...
1. Lockout start. Lying back on the decline bench with the head lower than the
   ...
   roller. Arms extended but not locked hard, the two dumbbells held directly
   over the lower chest roughly shoulder width apart and not touching.
...
3. Half descent, elbows at roughly 90 degrees, forearms vertical, dumbbells
   just above chest level, shoulder blades pinned back and down into the pad.
4. Bottom position. Dumbbells at chest level just outside the ribcage, handles
   ...
   The elbows do not drop far below the line of the bench. Chest lifted,
   glutes on the pad, shins still hooked behind the roller.
...
6. Lockout again, identical to frame 1, elbows extended, dumbbells over the
   lower chest and still roughly shoulder width apart.

TECHNIQUE — must be correct in every frame:
- The elbow clearly bends and extends across the frames — this is a press, not
  a wide arc with a fixed elbow angle.
- The bench reads clearly as roughly 15 degrees head-down in every frame, and
  the shins stay hooked behind the padded ankle roller.
...
- Wrists stay neutral and stacked over the elbows, forearms vertical at the
  bottom.
- Shoulder blades stay retracted into the pad, glutes stay down.
...
- [ ] Elbows clearly bend and extend — reads as a press, not a fly
- [ ] Elbows tucked to roughly 45 degrees, wrists stacked over elbows
- [ ] Dumbbells level with each other and never touching
- [ ] Same face, hair, wardrobe, and body proportions in all six frames
- [ ] Readable as a decline dumbbell press at 64 px wide
